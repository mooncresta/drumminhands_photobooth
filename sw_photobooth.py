...
button3_pin = 7 # pin for button to end the program, but not shutdown the pi
...
BRIGHTPI_ALLON = 'sudo i2cset -y 0 0x70 0x00 0xff'
BRIGHTPI_ALLOFF = 'sudo i2cset -y 0 0x70 0x00 0x00'
BRIGHTPI_TURNUP = 'sudo i2cset -y 0 0x70 0x09 0x0f'
	...
	if post_online: # turn off posting pics online in the variable declarations at the top of this document
		...
		while connected: 
			try:
				...
				file_to_upload = config.op_file_path + now + ".jpg"

				client.info() #show key
				client.create_photo(config.tumblr_blog, state="published", tags=["SallyPhotoBooth"], data=file_to_upload)
				break
			except ValueError:
